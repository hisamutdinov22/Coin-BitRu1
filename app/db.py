...
async def init_db() -> None:
    """Initialize missing tables and default tasks; safe to call on every restart."""
    try:
        async with engine.begin() as conn:
            await conn.run_sync(Base.metadata.create_all, checkfirst=True)
    except OperationalError as exc:
        # SQLite can report "table ... already exists" if two starts race.
        # The database is still usable, so continue and verify with a query below.
        msg = str(exc).lower()
