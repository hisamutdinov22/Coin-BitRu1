...
@app.post('/api/pay')
async def api_pay(body: PayIn, x_telegram_init_data: str = Header(default='')):
    u = await auth_user(x_telegram_init_data)
    if body.usd not in (1, 2, 5, 10):
        raise HTTPException(400, 'Доступные суммы: $1, $2, $5, $10.')
    if not settings.payment_provider_token:
        raise HTTPException(503, 'Платежи пока не настроены.')
    from aiogram import Bot
    bot = Bot(settings.bot_token)
    try:
        prices = [LabeledPrice(label=f'Coin BitRu ${body.usd}', amount=body.usd * 100)]
        invoice = await bot.create_invoice_link(
            title=f'Coin BitRu — пополнение ${body.usd}',
            description=f'Зачисление {body.usd * settings.coins_per_usd:,} COIN после подтверждения платежа.'.replace(',', ' '),
            payload=f'topup:{body.usd}:miniapp:{u.id}',
